...
def get_chart_data(request):
    ...
    expend_data = Expend.objects.values('created__month').filter(shop=request.user.shop, status='paid').annotate(
...
def extends_chart_data(request):
    expend_data = Expend.objects.annotate(month=TruncMonth('created')) \
    ...
    for data in expend_data:
        # Выводим для отладки
        print(f"Month: {data['month']}, Expend Type: {data['expend_type']}, Total Amount: {data['total_amount']}")
...
def create(request):
    ...
    expend_display_name = dict(Expend.EXPEND_TYPE_CHOICES).get(expend_type)
    
    # Инициализируем форму с начальным значением expend_type
    form = ExpendForm(initial={'expend_type': expend_type})
...
def expends(request):
    expends = Expend.objects.exclude(expend_type='salaries').filter(shop=request.user.shop).order_by('-id')
...
def expend_update(request, pk):
    # Находим объект Expend по первичному ключу (pk)
    expend = get_object_or_404(Expend, id=pk)
    ...
    form = ExpendForm(instance=expend, initial={'expend_type': expend_type})
...
def expend_delete(request, pk):
    # Настрой объект Expend по первичному ключу (pk)
    expend = get_object_or_404(Expend, id=pk)
    expend.delete()
    return redirect('expends')

def salary_list(request):
    salaries = Expend.objects.filter(expend_type='salaries', shop=request.user.shop)
